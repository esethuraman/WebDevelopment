defmodule TaskTrackerSPAWeb.TaskControllerTest do
  ...
  @create_attrs %{assigned_to: "some assigned_to", body: "some body", completed: true, description: "some description", minutes_worked: 42, name: "some name"}
  @update_attrs %{assigned_to: "some updated assigned_to", body: "some updated body", completed: false, description: "some updated description", minutes_worked: 43, name: "some updated name"}
  @invalid_attrs %{assigned_to: nil, body: nil, completed: nil, description: nil, minutes_worked: nil, name: nil}

  def fixture(:task) do
    {:ok, task} = App.create_task(@create_attrs)
    task
  end
  ...
  describe "update task" do
    ...
    test "renders task when data is valid", %{conn: conn, task: %Task{id: id} = task} do
      ...
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "assigned_to" => "some updated assigned_to",
